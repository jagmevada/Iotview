...
function showScheduleModal(sensorId, sensorData) {
      ...
      async function loadSchedulesFromDB(sensorId ) {
        try {
          const { data, error } = await window.supabase.from('schedule').select('*').eq('sensor_id', sensorId).order('id', { ascending: true });
          if (error) throw error;
          schedules = (data || []).map(row => {
            // parse timer_on_duration / timer_off_duration (HH:MM:SS) into on/off hour/min (24h)
            let onHour = null, onMinute = null, offHour = null, offMinute = null;
            ...
            let minute = (onMinute !== null) ? onMinute : (offMinute !== null ? offMinute : 0);
            const days = [];
            ...
            const onStr = (onHour !== null) ? `${onHour.toString().padStart(2,'0')}:${(onMinute||0).toString().padStart(2,'0')}` : null;
            const offStr = (offHour !== null) ? `${offHour.toString().padStart(2,'0')}:${(offMinute||0).toString().padStart(2,'0')}` : null;
      ...
      async function saveScheduleToDB(sched) {
        try {
          ...
          if (hh < 0) hh = 0; if (hh > 23) hh = hh % 24;
          const timeStr = `${hh.toString().padStart(2, '0')}:${mm.toString().padStart(2, '0')}:00`;
      ...
      function showEditScheduleModal(idx) {
        ...
        const existing = document.getElementById('edit-schedule-modal'); if (existing) existing.remove();
        const overlay = document.createElement('div'); overlay.id = 'edit-schedule-modal'; overlay.style.position='fixed'; overlay.style.top='0'; overlay.style.left='0'; overlay.style.width='100vw'; overlay.style.height='100vh'; overlay.style.background='rgba(0,0,0,0.45)'; overlay.style.zIndex='10001'; overlay.style.display='flex'; overlay.style.alignItems='center'; overlay.style.justifyContent='center';
        const box = document.createElement('div'); box.style.background='#23272f'; box.style.borderRadius='12px'; box.style.padding='18px'; box.style.width='92vw'; box.style.maxWidth='420px'; box.style.boxShadow='0 8px 32px rgba(0,0,0,0.25)'; box.style.position='relative';
        const header = document.createElement('div'); header.style.top='12px'; header.style.right='18px'; header.style.display='flex'; header.style.alignItems='center'; header.style.gap='12px'; box.appendChild(header);
        ...
        if (sched && sched.raw) {
          if (sched.raw.timer_on_duration) {
            ...
            defaultOnH = hhVal; defaultOnM = mmVal;
          }
          if (sched.raw.timer_off_duration) {
            ...
            defaultOffH = hhVal2; defaultOffM = mmVal2;
          }
        }
        // ON time
        const onWrap = document.createElement('div'); onWrap.style.display='flex'; onWrap.style.gap='8px'; onWrap.style.alignItems='center';
        const onLabel = document.createElement('div'); onLabel.textContent = 'ON'; onLabel.style.color = '#fff'; onLabel.style.minWidth = '36px'; onWrap.appendChild(onLabel);
        const onHH = document.createElement('select'); onHH.style.padding='8px'; onHH.style.borderRadius='8px';
        for (let i=0;i<24;i++) { const o = document.createElement('option'); o.value = i; o.textContent = i.toString().padStart(2,'0'); if (i === defaultOnH) o.selected = true; onHH.appendChild(o); }
        const onMM = document.createElement('select'); onMM.style.padding='8px'; onMM.style.borderRadius='8px'; [0,5,10,15,20,25,30,35,40,45,50,55].forEach(m=>{ const o=document.createElement('option'); o.value=m; o.textContent=m.toString().padStart(2,'0'); if (m===defaultOnM) o.selected=true; onMM.appendChild(o); });
        onWrap.appendChild(onHH); onWrap.appendChild(onMM);

        // OFF time
        const offWrap = document.createElement('div'); offWrap.style.display='flex'; offWrap.style.gap='8px'; offWrap.style.alignItems='center';
        const offLabel = document.createElement('div'); offLabel.textContent = 'OFF'; offLabel.style.color = '#fff'; offLabel.style.minWidth = '36px'; offWrap.appendChild(offLabel);
        const offHH = document.createElement('select'); offHH.style.padding='8px'; offHH.style.borderRadius='8px';
        for (let i=0;i<24;i++) { const o = document.createElement('option'); o.value = i; o.textContent = i.toString().padStart(2,'0'); if (i === defaultOffH) o.selected = true; offHH.appendChild(o); }
        const offMM = document.createElement('select'); offMM.style.padding='8px'; offMM.style.borderRadius='8px'; [0,5,10,15,20,25,30,35,40,45,50,55].forEach(m=>{ const o=document.createElement('option'); o.value=m; o.textContent=m.toString().padStart(2,'0'); if (m===defaultOffM) o.selected=true; offMM.appendChild(o); });
        offWrap.appendChild(offHH); offWrap.appendChild(offMM);

        timeRow.appendChild(onWrap); timeRow.appendChild(offWrap);
        ...
        const save = document.createElement('button'); save.type='button'; save.textContent='Save'; save.className='control-btn on'; save.onclick = async ()=>{
          // Prepare two schedule objects: ON and OFF
            ...
            const onTimeStr = `${onHour.toString().padStart(2,'0')}:${onMin.toString().padStart(2,'0')}:00`;
            const offTimeStr = `${offHour.toString().padStart(2,'0')}:${offMin.toString().padStart(2,'0')}:00`;
        ...
        const cancel = document.createElement('button'); cancel.type='button'; cancel.textContent='Cancel'; cancel.className='control-btn'; cancel.onclick = ()=>{ if(sched.id===-1) schedules = schedules.filter(s=>s!==sched); overlay.remove(); renderSchedules(); };
        buttons.appendChild(save); buttons.appendChild(cancel); box.appendChild(buttons);

        // close X
        const close = document.createElement('button'); close.textContent='×'; close.style.position='absolute'; close.style.top='8px'; close.style.right='10px'; close.style.background='none'; close.style.border='none'; close.style.color='#fff'; close.style.fontSize='1.3rem'; close.onclick=()=>{ if(sched.id===-1) schedules = schedules.filter(s=>s!==sched); overlay.remove(); renderSchedules(); };
